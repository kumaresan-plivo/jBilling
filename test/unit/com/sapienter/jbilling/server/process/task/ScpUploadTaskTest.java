...
public class ScpUploadTaskTest extends TestCase {
    ...
    public void testCollectFilesRecursive() throws Exception {
        ...
        assertEquals("itg_sub_headfoot_test.jasper", files.get(3).getName());
        assertEquals("itg_sub_test.jasper", files.get(4).getName());
        assertEquals("nges_invoice_design.jasper", files.get(5).getName());
        assertEquals("payment_notification_attachment.jasper", files.get(6).getName());
        assertEquals("simple_invoice.jasper", files.get(7).getName());
        assertEquals("simple_invoice_b2b.jasper", files.get(8).getName());
        assertEquals("simple_invoice_telco.jasper", files.get(9).getName());
        assertEquals("simple_invoice_telco_events.jasper", files.get(10).getName());
    }
    ...
    public void testCollectFilesCompoundRegex() throws Exception {
        ...
        assertEquals("simple_invoice_telco.jasper", files.get(9).getName());
        assertEquals("simple_invoice_telco_events.jasper", files.get(10).getName());
        assertEquals("entity-1.jpg", files.get(11).getName());
    }
