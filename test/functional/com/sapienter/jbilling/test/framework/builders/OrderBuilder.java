...
public class OrderBuilder extends AbstractBuilder{
    ...
    private Integer orderChangeStatusId = getOrCreateOrderChangeApplyStatus();
    ...
    public Integer persistOrder (OrderWS order) {
        OrderChangeWS[] orderChanges = buildFromOrder(order, orderChangeStatusId);
        Integer orderId = api.createOrder(order, orderChanges);
        testEnvironment.add(codeForTest, orderId, codeForTest, api, TestEntityType.ORDER);
        return orderId;
    }
    ...
    private OrderChangeWS[] buildFromOrder(OrderWS orderWS, Integer statusId) {
    ...
    private OrderWS findRootOrderIfPossible(OrderWS order) {
    ...
    private LinkedHashSet<OrderWS> findAllChildren(OrderWS rootOrder) {
        LinkedHashSet<OrderWS> orders = new LinkedHashSet<OrderWS>();
        findChildren(rootOrder, orders);
        return orders;
    }

    private void findChildren(OrderWS order, LinkedHashSet<OrderWS> orders) {
