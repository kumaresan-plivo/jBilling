...
//            assertEquals("Invalid method Id!", paymentInstrument.getPaymentMethodId(), Constants.PAYMENT_METHOD_VISA);
    ...
    public void testProductCategoryCreationAndCleanAfterTest() {
        TestEnvironment testEnv = TestBuilder.newTest().given(envBuilder -> {
            JbillingAPI api = envBuilder.getPrancingPonyApi();
            envBuilder.itemBuilder(api).itemType().withCode("testCategory").build();
        }).test((env) -> {
            JbillingAPI api = env.getPrancingPonyApi();
        ...
        try {
            testEnv.getPrancingPonyApi().getItemCategoryById(testEnv.idForCode("testCategory"));
            fail("Search Item Type should throw Session Internal exception when deleted");
        } catch (NullPointerException e){
        } catch (Exception e) {
            fail("Search Item Type throw a Null Pointer Exception if is not in the system");
        }
    }
