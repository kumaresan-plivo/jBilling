...
public class CollectionsTest {
    ...
    public void tearDown(){
        ...
        api.saveAgeingConfiguration(new AgeingWS[0], api.getCallerLanguageId());
    ...
    private TestBuilder getPluginsAndItemSetup(){
        return TestBuilder.newTest().givenForMultiple(envCreator -> {
            ...
            api.saveAgeingConfiguration(buildAgeingSteps(api), api.getCallerLanguageId());
            createPluginConfig(api, configurationBuilder, findStepIdByStatus(api.getAgeingConfiguration(api.getCallerLanguageId()), GRACE_PERIOD_STEP),
            ...
            Integer paymentTypeId = envCreator.paymentMethodTypeBuilder(api, PAYMENT_TYPE_CODE)
                    ...
                    .allAccountType(true)
                    .addMetaField(ApiBuilderHelper.getMetaFieldWithValidationRule(CC_CARD_HOLDER_NAME_MF, DataType.STRING, EntityType.PAYMENT_METHOD_TYPE,
                            api.getCallerCompanyId(), MetaFieldType.TITLE, null))

                    .addMetaField(ApiBuilderHelper.getMetaFieldWithValidationRule(CC_NUMBER_MF, DataType.STRING, EntityType.PAYMENT_METHOD_TYPE,
                    ...
                    .addMetaField(ApiBuilderHelper.getMetaFieldWithValidationRule(CC_EXPIRY_DATE_MF, DataType.STRING, EntityType.PAYMENT_METHOD_TYPE,
                    ...
                    .addMetaField(ApiBuilderHelper.getMetaFieldWithValidationRule(CC_TYPE_MF, DataType.INTEGER,
                            EntityType.PAYMENT_METHOD_TYPE, api.getCallerCompanyId(), MetaFieldType.CC_TYPE, null))

                    .addMetaField(ApiBuilderHelper.getMetaFieldWithValidationRule(CC_GATEWAY_KEY_MF, DataType.STRING,
    ...
    public void test001CollectionStepCheckTest(){
        ...
        testBuilder.test((env) -> {
            ...
            AgeingWS[] ageingList = api.getAgeingConfiguration(api.getCallerLanguageId());
    ...
    public void test006SApplyLateFeeOnCollectionStep(){
        ...
        testBuilder.given(envBuilder ->{
            ...
            createFeePluginConfig(api, configurationBuilder,
                    findStepIdByStatus(api.getAgeingConfiguration(api.getCallerLanguageId()), FIRST_RETRY_STEP), feeProductId);
            configurationBuilder.build();
            updateBillingProcessConfiguration(activeSince, api);
            triggerBillingProcess(activeSince, api);
            api.triggerAgeing(new LocalDate(activeSince).plusDays(1).toDate());
    ...
    private Integer createCustomer(TestEnvironmentBuilder envBuilder, String code, Integer accountTypeId, Date nid, Integer paymentTypeId, boolean goodCC){
        ...
        if (null != paymentTypeId){
            String date = DateTimeFormat.forPattern(Constants.CC_DATE_FORMAT).print(Util.truncateDate(new LocalDate().plusYears(10).toDate()).getTime());
            customerBuilder.addPaymentInstrument(
                    customerBuilder.paymentInformation()
                            ...
                            .addMetaFieldValue(ApiBuilderHelper.getMetaFieldValueWS(CC_CARD_HOLDER_NAME_MF, CUSTOMER_CODE))
                            .addMetaFieldValue(ApiBuilderHelper.getMetaFieldValueWS(CC_NUMBER_MF, goodCC ? "4111111111111152" : "4111111111111111"))
                            .addMetaFieldValue(ApiBuilderHelper.getMetaFieldValueWS(CC_EXPIRY_DATE_MF, date))
                            .addMetaFieldValue(ApiBuilderHelper.getMetaFieldValueWS(CC_TYPE_MF, Constants.PAYMENT_METHOD_VISA))
                            .addMetaFieldValue(ApiBuilderHelper.getMetaFieldValueWS(CC_GATEWAY_KEY_MF, "zzzxxxaaa"))
                            .build()
            );
        }
    ...
    private AgeingWS[] buildAgeingSteps(JbillingAPI api) {
        ...
        ageingSteps[0] = buildAgeingStep(PAYMENT_DUE_STEP, Integer.valueOf(0), false, false, false, api);
        ageingSteps[1] = buildAgeingStep(GRACE_PERIOD_STEP, Integer.valueOf(2), false, true, false, api);
        ageingSteps[2] = buildAgeingStep(FIRST_RETRY_STEP, Integer.valueOf(3), true, false, false, api);
        ageingSteps[3] = buildAgeingStep(SUSPENDED_STEP, Integer.valueOf(7), false, false, true, api);
        return ageingSteps;
    }
    ...
    private AgeingWS buildAgeingStep(String statusStep,Integer days,
                                     boolean payment , boolean sendNotification, boolean suspended, JbillingAPI api){
        ...
        ageingWS.setWelcomeMessage("Welcome Message" + System.currentTimeMillis());
        ageingWS.setFailedLoginMessage("You failed" + System.currentTimeMillis());
        return  ageingWS;
    }
