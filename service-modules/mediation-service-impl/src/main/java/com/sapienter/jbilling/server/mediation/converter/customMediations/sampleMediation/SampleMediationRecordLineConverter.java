 ...
 */
public class SampleMediationRecordLineConverter extends BasicMediationRecordLineConverter implements LineMapper<CallDataRecord> {

    private Format format;

    @Override
    public CallDataRecord convertLineToRecord(String line) {
        setDateFormat(DateTimeFormat.forPattern("yyyy-MM-dd HH:mm:ss"));
        return super.convertLineToRecord(line, this.format);
    }

    @Override
    public CallDataRecord convertLineToRecord(String line, Format format) {
        setDateFormat(DateTimeFormat.forPattern("yyyy-MM-dd HH:mm:ss"));
        if (this.format != null) {
            return super.convertLineToRecord(line, this.format);
        } else {
    ...
    @Override
    public CallDataRecord mapLine(String line, int lineNumber) throws Exception {
