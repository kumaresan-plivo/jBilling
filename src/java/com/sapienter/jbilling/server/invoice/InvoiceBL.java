...
public class InvoiceBL extends ResultList implements Serializable, InvoiceSQL {
    ...
    public void delete(Integer executorId) throws SessionInternalError {
        ...
        PartnerCommissionDAS pcd = new PartnerCommissionDAS();
        List<InvoiceCommissionDTO> invoiceCommissions = pcd.findInvoiceCommissionByInvoice(invoice.getId());
        if (!invoiceCommissions.isEmpty()) {
            SessionInternalError sie= new SessionInternalError("An Invoice referenced to a commission cannot be deleted");
            sie.setErrorMessages(new String[] {
                    "invoice.error.commission.fkconstraint," + invoice.getId()});
            throw sie;
        }
    ...
    public CachedRowSet getPayableInvoicesByUserOldestFirst(Integer userId)
            throws SQLException, Exception {
