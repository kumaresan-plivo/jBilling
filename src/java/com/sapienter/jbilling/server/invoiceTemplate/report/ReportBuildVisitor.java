...
public class ReportBuildVisitor implements Visitor {
    ...
    private final Map<String, String> subDataSources = new HashMap<String, String>();
    private final Map<String, JRDesignDataset> subDataSets = new HashMap<String, JRDesignDataset>();

    private final Map<String, SubReportDataSource> subReportDataSources = new HashMap<String, SubReportDataSource>();
    ...
    public ReportBuildVisitor(Map<Class<?>, Object> resources, Map<String, Class<?>> parameters, Map<String, String> dynamicParameters) {
        this.resources = new HashMap<Class<?>, Object>(resources);
        this.parameters = new HashMap<String, Class<?>>(parameters);
        this.dynamicParameters = new HashMap<String, String>(dynamicParameters);
        this.xFields = new HashMap<String, Class<?>>();
        rootDesign = createJasperDesign();
        rootDesign.setWhenNoDataType(WhenNoDataTypeEnum.ALL_SECTIONS_NO_DETAIL);

        subReports = new HashMap<String, JasperDesign>();
        sections = new HashMap<String, JasperDesign>();
    ...
    public void accept(SubReport subReport) {
        ...
        try {
            currentSubReport = JRXmlLoader.load("descriptors" + File.separator + "designs" + File.separator + subReport.fileName + ".jrxml");
    ...
    public void accept(EventLines eventLines) {
        ...
        ColumnSettings.setupLines(eventLines, columnHeader, jrDesignBand, columnWidth);
    ...
    public void accept(InvoiceLines invoiceLines) {
        ...
        JRDesignBand jrDesignBand = createBand(PREVENT);
        ((JRDesignSection) currentSubReport.getDetailSection()).addBand(jrDesignBand);
        ColumnSettings.setupLines(invoiceLines, columnHeader, jrDesignBand, columnWidth);
        JRElement[] elements = columnHeader.getElements();
        int hdrWidth;
        if (elements.length > 0) {
            int last = elements.length - 1;
    ...
    private static JRDesignGroup createGroup(TableLines lines, FieldSetup fieldSetup, String formatter, int width, boolean isMainGroup) throws JRException {
        ...
        JRDesignExpression expression = fieldSetup.createExpression(lines, formatter);
    ...
    public InvoiceTemplateBL createInvoiceTemplateBL() {
        Map<String, JasperDesign> allSubreports = new HashMap<String, JasperDesign>(sections);
