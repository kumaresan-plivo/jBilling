...
public class ConvertUtils {
    ...
    //TODO MODULARIZATION: WHAT IS THIS METHOD??
    public static List<CdrEnvelope> collectCdrs(Map<JbillingMediationRecord, CallDataRecord> mediationRecordLines, Locale locale, String defaultAssetIdLabel) {
        ...
        ArrayList<CdrEnvelope> result = new ArrayList<CdrEnvelope>(mediationRecordLines.size());
        int cdrCounter = 0;
        for (Map.Entry<JbillingMediationRecord, CallDataRecord> entry : mediationRecordLines.entrySet()) {
            ...
            OrderLineDTO orderLineDTO = orderLineDAS.find(orderLineId);
            ItemDTO item = orderLineDTO.getItem();

            Set<AssetEnvelope> cdrAssets = new TreeSet<AssetEnvelope>(new Comparator<AssetEnvelope>() {
                @Override
