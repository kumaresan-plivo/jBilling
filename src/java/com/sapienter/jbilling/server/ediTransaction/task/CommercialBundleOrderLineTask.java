        ...
        try {
            if(event instanceof  NewOrderEvent){
                ...
                for(OrderLineDTO orderLineDTO:orderDTO.getLines()){
                    if(orderLineDTO.getItem().isPlan()){
                        containsPlan=true;
                        return;
                    }
                }
                ...
                MetaFieldValue zoneMetaField=meterReadOrderCreationEvent.getOrder().getUser().getCustomer().getMetaField(FileConstants.CUSTOMER_ZONE_META_FIELD_NAME);
