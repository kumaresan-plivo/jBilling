...
public class RateChangeTask extends AbstractCronTask {
    ...
    public void doExecute(JobExecutionContext context) throws JobExecutionException {
        ...
        List<UserDTO> userDTOList=new UserDAS().findByMetaFieldNameAndValue(FileConstants.CUSTOMER_RATE_CHANGE_DATE_META_FIELD_NAME, dateTime.toDateMidnight().toDate(), getEntityId());
        for(UserDTO tmpUserDTO:userDTOList){
            ...
            try{
                ...
                UserDTO userDTO = new UserDAS().find(tmpUserDTO.getId());

                MetaFieldValue rateMetaFieldValue=userDTO.getCustomer().getMetaField(ChangeRequestGenerationConstants.RATE.getValue());
                ...
                if(metaFieldValue.getValue().equals(ChangeRequestGenerationConstants.RATE_READY.getValue())){
                    ...
                    if(planItemPrice.compareTo((BigDecimal)rateMetaFieldValue.getValue())!=0){
                        ...
                        setCommonParameter();
                        Map<String, String> metaFieldMap = fetchMapForMetaField(getCustomerMetaFieldValues(userDTO));
                        mapMetaFieldWithRecord(metaFieldMap, userDTO.getEntity().getId(), userDTO);
                ...
                //TODO file should be generated in all the condition. In this case file status will be error
                LOG.error("Unable to process user "+ tmpUserDTO, e);
    ...
    private void mapMetaFieldWithRecord(Map<String, String> metaFieldMap, Integer entityId, UserDTO user) {
...
//        Create file naming convention using this link. https://docs.google.com/document/d/14D0T2jQeMajCD3xnWu2_2cYGy6gEHL6q7mRIzrgxACA/edit
        ...
        try {
            EDIFileDTO ediFileDTO = ediTransactionBean.generateEDIFile(fileFormat, entityId, FILE_NAME, recordMapList);
    ...
    private Map<String, String> setHeaderRecord(Record record, Map<String, String> metaFieldMap) {
        ...
        headerMap.put(CREATE_DATE, convertStringToDate(convertDateToString(new Date()), getDateFormatFromRecord(record, CREATE_DATE)));
        return headerMap;
    }
