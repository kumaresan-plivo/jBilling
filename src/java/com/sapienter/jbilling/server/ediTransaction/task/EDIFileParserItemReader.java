...
//    private String transactionSet;
    ...
    private File[] pickFiles(String utilityDUNS, String supplierDUNS, String transactionSet) {
        ...
        File dir = new File(FileConstants.COMMUNICATION_INBOUND_PATH);
       if(!dir.exists()){
           LOG.debug("Directory not exist :  "+FileConstants.COMMUNICATION_INBOUND_PATH);
           throw new SessionInternalError("Directory not exist :  "+FileConstants.COMMUNICATION_INBOUND_PATH);
       }
    ...
    private File moveFileToInboundFolder(File movedFile) throws IOException {
        ...
        File file = new File(FileConstants.getEDITypePath(fileFormat.getEdiTypeDTO().getEntity().getId(), fileFormat.getEdiTypeDTO().getPath(), FileConstants.INBOUND_PATH) + File.separator + movedFile.getName());
        if (movedFile.exists()) {
            try{
