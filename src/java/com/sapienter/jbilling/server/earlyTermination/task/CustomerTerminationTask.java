...
//            Check if TRANS_REF_NR is unique
            ...
            ediTransactionBean.isUniqueKeyExistForFile(getEntityId(), ediTypeId, ediFileWS.getId(), CustomerTerminationField.TRANS_REF_NR.toString(), transferNRValue, TransactionType.INBOUND);
        } catch (Exception ex) {
        ...
        parseRecordsList(ediFileWS.getEDIFileRecordWSes());
...
//        Get User from db if it exist for file.
...
//        EDIFileWS ediFileWS = orderSessionBean.getEDIFileWS(ediFileDTO);
        ...
        File targetFile = new File(FileConstants.COMMUNICATION_OUTBOUND_PATH + File.separator + generatedFileWS.getName());
        FileUtils.copyFile(sourceFile, targetFile);
    }

    private UserWS getUserIfExist() {
        String terminationCustomerAccountValue = findField(CustomerTermination.ACT, CustomerTerminationField.UTILITY_CUST_ACCT_NR.toString());
        if (blank(terminationCustomerAccountValue)) {
            status = INVALID_FILE_STATUS;
        ...
        try{
             userDTO = ediTransactionBean.findUserByAccountNumber(companyId, CustomerTerminationField.UTILITY_CUST_ACCT_NR.toString(), terminationCustomerAccountValue);
