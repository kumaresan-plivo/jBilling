...
public class OrderBL extends ResultList
    ...
    private static void updateOrderOwnFields(OrderDTO targetOrder, OrderDTO updatedOrder, boolean updateAuditedFields) {
        ...
        targetOrder.updateMetaFieldsWithValidation(targetOrder.getBaseUserByUserId().getCompany().getLanguageId(),
                targetOrder.getBaseUserByUserId().getCompany().getId(), null, updatedOrder);
     ...
     */
    public static final BigDecimal getTotalWithPlansAsDecimal(OrderWS ws,Integer languageId, Integer userId, Integer entityId, Integer currencyId) {
                ...
                if (planDTO.getEditable() == 0) {
                    for (PlanItemDTO planItemDTO: planDTO.getPlanItems()) {
                        ...
                        if (itemDTO.isPercentage()) {
                            ...
                            if (planItemDTO.getBundle() != null && planItemDTO.getBundle().getQuantity() != null) {
                                lineTotal = lineTotal.add(planItemDTO.getPrice(new Date()).getRate()
                                        .multiply(planItemDTO.getBundle().getQuantity()));
                            }
                        }
                    }
    ...
    public void addItem(OrderDTO order, Integer itemID, BigDecimal quantity, Integer language, Integer userId,
        ...
        try {
            ...
            if ( null != itemAdded && !itemAdded.getPlans().isEmpty()) {
                ...
                if (!PlanBL.isSubscribed(userId, itemID, new Date())) {
                    PlanBL.subscribe(userId, itemID, new Date(), null);
                }
            }
    ...
    private void deleteOrder(OrderDTO order, Integer executorId) {
        ...
        List<CustomerPlanUnsubscriptionEvent> customerPlanUnsubscriptionEvents = new ArrayList<CustomerPlanUnsubscriptionEvent>();
	        ...
	        if (null != line.getItem() && line.getItem().hasPlans()) {
            	customerPlanUnsubscriptionEvents.add(
            		new CustomerPlanUnsubscriptionEvent(
            			...
            			order, Constants.CUSTOMER_PLAN_UNSUBSCRIBE_ORDER_DELETE)
            	);
            }
        }
        order.setDeleted(1);
        ...
        LOG.debug("OrderBL.delete(): generated SubscriptionInactiveEvent for order: %s", order.getId());
...
//    }
    ...
    private void processCustomerPlanSubscription(List<OrderLineDTO> lines, Integer userId) {

    	for (OrderLineDTO line : lines) {
    		if (null != line.getItem() && null != line.getItem().getPlans() &&
    			!line.getItem().getPlans().isEmpty() && line.getDeleted() == 0) {

    			CustomerPlanSubscriptionEvent newEvent =
    					new CustomerPlanSubscriptionEvent(order.getBaseUserByUserId().getEntity().getId(), line);
    			EventManager.process(newEvent);
    		}
    	}
    }
        ...
        try {
            ...
            for (OrderLineDTO line : order.getLines()) {
                ...
                if ( null != lineItemID && itemDAS.findNow(lineItemID).getPlans().size() > 0 ) {
                	line.setAssets(new HashSet<AssetDTO>());
                }
            ...
            if (CollectionUtils.isNotEmpty(order.getDiscountLines())) {
            	...
            	order.setDiscountLines(new LinkedList<DiscountLineDTO>());
            }
            ...
            processCustomerPlanSubscription(order.getLines(), order.getUserId());
    ...
    public void updateActiveUntil(Integer executorId, Date to, OrderDTO newOrder) {
        ...
        if (to == null || to.before(now)) {
            ...
            EventManager.process(newEvent);
            LOG.debug("OrderBL.updateActiveUntil(): generated SubscriptionInactiveEvent for order: %s", order.getId());
        }
    ...
    public List<Integer> update(Integer executorId, OrderDTO dto, Map<OrderLineDTO, OrderChangeDTO> appliedChanges) {
        ...
        LOG.debug("OrderBL.update");
        LOG.debug("OrderDTO %s", dto);
        for (OrderLineDTO orderLineDTO : appliedChanges.keySet()) {
            LOG.debug("OrderLineDTO %s", orderLineDTO);
            LOG.debug("OrderChangeDTO %s", appliedChanges.get(orderLineDTO));
        }

    	List<CustomerPlanUnsubscriptionEvent> customerPlanUnsubscriptionEvents = 
    			new ArrayList<CustomerPlanUnsubscriptionEvent>();
            ...
            for (OrderLineDTO line : dto.getLines()) {
            	if (null != line.getItem() && line.getItem().hasPlans()) {
            		customerPlanUnsubscriptionEvents.add(
                    		new CustomerPlanUnsubscriptionEvent(
                    			order.getBaseUserByUserId().getEntity().getId(),
                    			order, Constants.CUSTOMER_PLAN_UNSUBSCRIBE_UPDATE_ACTIVE_UNTIL)
                    	);

                    //8922 - update customer prices expiry date possibly
                    LOG.debug("Active Until updated. Will expire prices for Plans subscriptions");
        ...
        if (!Util.equal(order.getActiveSince(), dto.getActiveSince())) {
            ...
            for (OrderLineDTO line : dto.getLines()) {
                if (null != line.getItem() && line.getItem().hasPlans()) {
                    ...
                    LOG.debug("Active Since updated. Will refresh priceSubscriptionDate for customer's Plans subscriptions");
                    ItemDTO plan= new ItemDAS().findNow(line.getItemId());
                    if ( null != plan && plan.getPlans().iterator().hasNext()) {
                        PlanDTO thePlan= plan.getPlans().iterator().next();
        ...
        List<OrderLineDTO> newLines = new LinkedList<OrderLineDTO>(order.getLines());
        ...
        for (OrderLineDTO line : newLines) {
            ...
            if (dtoLine != null && dtoLine.getId() == 0) dtoLine.setId(line.getId());
        }

    /* We already did this 'mili' seconds back in call to updateOrderLinesInOrder.
        A plan is not removed by replacing oldLines with new lines. A plan is removed by marking an existing line as deleted
    ...
    */
        ...
        // but in future we can have many plans on one order. Fire event for each plan.
        LOG.debug("Before firing CustomerPlanUnsubscriptionEvent");
    	for (CustomerPlanUnsubscriptionEvent event: customerPlanUnsubscriptionEvents) {
    		LOG.debug("Firing CustomerPlanUnsubscriptionEvent");
    		EventManager.process(event);
    	}
    ...
    private List<Integer> updateOrderLinesInOrder(
        ...
        for (OrderLineDTO line : updatedLineDtos) {
            if (line.getId() <= 0) {
                ...
                // reset children info in lines - saving only via parent
                line.getChildLines().clear();
                Set<AssetDTO> newAssets = new HashSet<AssetDTO>(line.getAssets());
                line.getAssets().clear();
    ...
    private void addCustomerPlans(List<OrderLineDTO> lines, Integer userId, Date activeSince, Date activeUntil) {
        ...
        for (OrderLineDTO line : lines) {
            ...
            if (line.getItem() !=null && !line.getItem().getPlans().isEmpty() && line.getDeleted() == 0) {
                ...
                Date startDate = ( null != activeSince ? activeSince : (null != line.getStartDate() ? line.getStartDate() : new Date()) );
    ...
    private void addOrRemoveCustomerPlans(List<OrderLineDTO> lines, Integer userId, Map<OrderLineDTO, OrderChangeDTO> appliedChanges) {
        ...
        for (OrderLineDTO line : lines) {
            ...
            if (line.getItem() !=null && !line.getItem().getPlans().isEmpty() ) {
                ...
                Date effectiveDate= (null != line.getStartDate() ? line.getStartDate() : new Date()) ;
                Date endDate = (null != line.getEndDate() ? line.getEndDate() : new Date());
                ...
                if ( line.getDeleted() == 0 ) {
                    ...
                    PlanBL.unsubscribe(userId, line.getItemId(), effectiveDate);
                }
            }
        }
    }
    ...
    private void removeCustomerPlans(List<OrderLineDTO> lines, Integer userId) {
        ...
        for (OrderLineDTO line : lines) {
            ...
            if (line.getItem() != null && !line.getItem().getPlans().isEmpty()) {
                if (!PlanBL.isSubscribed(userId, line.getItemId(), new Date())) {
    ...
    private List<Integer> addBundledItems(OrderDTO order, List<OrderLineDTO> lines,
        ...
        for (OrderLineDTO line : lines) {
            ...
            Set<PlanItemDTO> allOrdersPlanItems = new HashSet<PlanItemDTO>();
            if (line.getDeleted() == 0 && line.getItem() != null && line.getItem().isPlan()) {
                Set<OrderDTO> ordersForCurrentPlan = new HashSet<OrderDTO>();
                PlanDTO plan = getPlanForItem(line.getItem());
                String planLineOrderKey = baseUser.getId() + "_" + plan.getPeriod().getId();
                if (line.getId() <= 0 && !planLineOrderKey.equals(currentOrderKey)) {  // new order line with incorrect period for current order
                    ...
                    if (!orders.containsKey(planLineOrderKey)) {
                        LOG.debug("Getting bundle order for user %s and period %s", baseUser.getId(), plan.getPeriod().getId());
...
//                    LOG.debug("line %s\norder %s", line, order);
                    //                    OrderDTO lineTargetOrder = orders.get(planLineOrderKey);
                    //                    line.setPurchaseOrder(lineTargetOrder);
                    //                    lineTargetOrder.getLines().add(line);
                    //                    order.getLines().remove(line);
                    // this order line will be processed during new orders processing
                }  else {
                        ...
                        if (planItem.getBundle() != null && planItem.getBundle().getQuantity().compareTo(BigDecimal.ZERO) > 0) {
                            if ( planItem.getBundle().getPeriod().getId() != Constants.ORDER_PERIOD_ALL_ORDERS ) {
                            	if (!quantityDifference.equals(BigDecimal.ZERO)) {
                            		...
                            		LOG.debug ("No quantity to add/remove, quantityDifference %s", quantityDifference);
                            	}
                            } else {
    ...
    private OrderDTO addPlanItemToAppropriateOrder(PlanItemDTO planItem, BigDecimal quantityDifference,
        ...
        String mapKey = user.getId() + "_" + targetPeriod.getId();
    ...
    private void createOrderChangesForBundledLineItems(Map<OrderLineDTO, OrderChangeDTO> appliedChanges, OrderDTO bundledOrder) {
        for (OrderLineDTO line: bundledOrder.getLines()) {
            if (!line.getItem().isPlan()) {
                if (line.getOrderChanges() == null) line.setOrderChanges(new HashSet<>());
                if (line.getOrderChanges().size() == 0) {
    ...
    private BigDecimal getDiscountableAmount(OrderDTO orderDto, DiscountLineDTO discountLine, boolean create) {
        ...
        if (discountLine.isPlanItemLevelDiscount()) {
        	if (create && null != orderDto) {
                ...
                LOG.debug("Found planSubscriptionLine for discount line %s as %s", discountLine, planSubscriptionLine);
                ...
                LOG.debug("Order Line's quantity is %s whereas discountLine.planItem.bundleQty is %s ", ( null != planItemLine ? planItemLine.getQuantity(): null), discountLine.getPlanItem().getBundle().getQuantity());

                if(item.getPrice(new Date()).getType() == PriceModelStrategy.LINE_PERCENTAGE) {
	        	    LOG.warn("The discount on a percentage Plan Item.");
	        	    ...
	        	    if ( null != planItemPrice ) {
	            	    LOG.debug("Plan Item Rate/Price for " + today + " is " + planItemPrice.getRate());
...
//          orderLineDAS.delete(line);
    ...
    public OrderDTO getDTO(OrderWS order, Map<OrderWS, OrderDTO> ordersHierarchyMap, Map<OrderLineWS, OrderLineDTO> orderLinesHierarchyMap) {
        ...
        if (retValue == null) {
            ...
            if (order.getProvisioningCommands() != null) {
                List <OrderProvisioningCommandDTO> commandDTOList = new ArrayList<OrderProvisioningCommandDTO>();
                for (ProvisioningCommandWS command : order.getProvisioningCommands()){
                       if (command != null && command.getProvisioningRequests() != null){
                           List <ProvisioningRequestDTO> requestDTOList = new ArrayList<ProvisioningRequestDTO>();
	...
	public void processLines(OrderDTO order, Integer languageId,
		...
		for (OrderLineDTO line : order.getLines()) {
			...
			if (line.getUseItem()) {
                ...
                if(!line.getItem().isPlan() || (line.getId() == 0 && line.getItem().isPlan())) {
    ...
    public void setOrderProvisioningStatus(Integer provisioningStatus) {
        ...
        LOG.debug("order %s: updated provisioning status :%s", order, order.getProvisioningStatusId());
    ...
    public List<OrderWS> findOrdersByUserPagedSortedByAttribute(Integer userId, int maxResults, int offset, String sortAttribute, ListField.Order order, Integer callerLanguageId) {
        ...
        List<OrderWS> ordersWS = new ArrayList<OrderWS>();
        OrderDTO dto = null;
