...
public class UserBL extends ResultList implements UserSQL {
    ...
    public void update (Integer executorId, UserDTOEx dto) throws SessionInternalError {
        ...
        if (dto.getCustomer() != null && user.getCustomer() != null) {
            ...
            try {
                ...
                for(PartnerDTO partner : dto.getCustomer().getPartners()) {
            ...
            if (parentChildCustomerPreference == 1 && parent != null)
                parentCustomerOrderPeriodDTO= parent
                    .getMainSubscription()
                    .getSubscriptionPeriod();
            ...
            if (dto.getCustomer().getPartners() != null) {
                try {
                    ...
                    for(PartnerDTO dtoPartner : dto.getCustomer().getPartners()) {
                        ...
                        if (newPartner.getUser().getEntity().getId() == dto.getEntityId()
                                && newPartner.getUser().getDeleted() == 0) {
    ...
    public void sendDeletedUser(Integer entityId, Integer userId,
        ...
        try{
            ...
            LOG.error("Exception while sending notification : " + e.getMessage());
            throw new SessionInternalError("Notification not found for sending deleted user notification");
        }
    }
    ...
    public void delete (Integer executorId) throws SessionInternalError {
        ...
        try {
            sendDeletedUser(user.getCompany().getId(), user.getId(), 1);
    ...
    private static List<Integer> getAccessEntities(UserDTOEx dto) {
        ...
        Integer entityId = dto.getEntity().getParent() != null ? dto.getEntity().getParent().getId() : dto.getEntity().getId();
        return companyDAS.getCurrentAndDescendants(entityId);
    }
    ...
    public void setCustomerNextInvoiceDate (UserDTO userDto) {
        ...
        BillingProcessConfigurationDTO billingProcessConfiguration = new BillingProcessConfigurationDAS().findByEntity(
                userDto.getEntity());
        ...
        if (cal.getActualMaximum(Calendar.DAY_OF_MONTH) <= customerDayOfInvoice
            ...
            while (Util.truncateDate(billingProcessConfiguration.getNextRunDate()).after(Util.truncateDate(cal.getTime()))) {
            ...
            if (mainSubscriptionPeriodUnit.equals(Constants.PERIOD_UNIT_MONTH)) {
                ...
                if (dayOfWeek > customerDayOfInvoice) {
                    int day = 7 - (dayOfWeek - customerDayOfInvoice);
                    cal.add(Calendar.DAY_OF_MONTH, day);
                } else if (dayOfWeek < customerDayOfInvoice) {
                    int day =  customerDayOfInvoice - dayOfWeek;
                    cal.add(Calendar.DAY_OF_MONTH, day);
                }

                if (Util.truncateDate(billingProcessConfiguration.getNextRunDate()).after(Util.truncateDate(cal.getTime()))) {
                    cal.setTime(billingProcessConfiguration.getNextRunDate());
                    if (cal.get(Calendar.DAY_OF_WEEK ) < customerDayOfInvoice) {
                        int day = customerDayOfInvoice - cal.get(Calendar.DAY_OF_WEEK);
                        cal.add(Calendar.DAY_OF_MONTH, day);
                    }
                }
            ...
            } else if (mainSubscriptionPeriodUnit.equals(Constants.PERIOD_UNIT_DAY)) {
                cal.set(Calendar.DAY_OF_MONTH, customerDayOfInvoice);
                while (Util.truncateDate(billingProcessConfiguration.getNextRunDate()).after(Util.truncateDate(cal.getTime()))) {
                    cal.setTime(billingProcessConfiguration.getNextRunDate());
                }
    ...
    public void updateMetaFieldsWithValidation(Integer entityId, Integer accountTypeId, MetaContent dto) {
        ...
        customerDTO.updateMetaFieldsWithValidation(new CompanyDAS().find(entityId).getLanguageId(), entityId,
                accountTypeId, dto);
    ...
    public CustomerDTO getUserByPrimaryAsset(String primaryAsset) {
        CustomerDAS das = new CustomerDAS();
        Integer customerId = das.getCustomerIdByPrimaryAsset(primaryAsset);
        return das.find(customerId);
    }
}
