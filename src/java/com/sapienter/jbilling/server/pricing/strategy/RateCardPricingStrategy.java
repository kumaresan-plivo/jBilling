...
import com.sapienter.jbilling.server.pricing.cache.*;
...
public class RateCardPricingStrategy extends AbstractPricingStrategy {
    ...
    private static final String PRICE_TERM = "price_term";
    ...
    public void applyTo(OrderDTO pricingOrder, PricingResult result, List<PricingField> fields, PriceModelDTO planPrice,
        ...
        if (curr != null) {
            ...
            if (currency != null && applyToCurrency != null && !currency.equals(applyToCurrency)) {
                LOG.debug("Request currency %s does not match card currency %s, skipping rate card lookup",
                ...
                if (pricingFinder != null) {
                    ...
                    MatchCallback callback = new MatchCallback() {
                        ...
                        public BigDecimal onMatch(SqlRowSet set) {
                            ...
                            if (columnExists(PRICE_TERM, set)) {
                                String term = set.getString(PRICE_TERM);
                                if ("free".equalsIgnoreCase(term)) {
                                    return new FreePriceBigDecimal();
                                } else if ("n/a".equalsIgnoreCase(term)) {
                                    return new UnavailablePriceBigDecimal();
                                }
                            }
