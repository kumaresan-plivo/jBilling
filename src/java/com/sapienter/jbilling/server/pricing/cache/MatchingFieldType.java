    ...
    BEST_MATCH () {
        ...
        public void apply (final MatchingFieldDTO matchingFieldDTO, final PricingField inputParameter, final MatchingFieldResult matchingFieldResult) {
            if (inputParameter != null) {
                matchingFieldResult.addBestMatchCriteria(matchingFieldDTO.getMatchingField(), inputParameter.getStrValue(), 
                        matchingFieldDTO.getSmallestValue(), matchingFieldDTO.getLongestValue());
            }
        }
    },
                ...
                public boolean accept(MatchingFieldDTO matchingFieldDTO, MatchingRecord record) throws Exception {
                    ...
                    if (dateRange.length > 0) {
                        ...
                        if (dateRange.length > 1) {
                            ...
                            endTime = new DateMidnight().toDate();
                        }
                    }
        ...
        public void apply(final MatchingFieldDTO matchingFieldDTO, final PricingField inputParameter, final MatchingFieldResult matchingFieldResult) {
            ...
            if (inputObject instanceof Date) {
                ...
                try {
                    ...
                    throw new SessionInternalError("Cannot parse input event date", e);
                }
            }
            ...
            if (eventDate != null) {
                ...
                matchingFieldResult.addFilterCriteria(matchingFieldDTO, new FilterCallback() {
                    ...
                    public boolean accept(MatchingFieldDTO matchingFieldDTO, MatchingRecord record) throws Exception {
                        ...
                        if (dateRange.length > 0) {
                            startDate = dateFormat.parseDateTime(dateRange[0]).toDate();

                            if (dateRange.length > 1) {
                                endDate = dateFormat.parseDateTime(dateRange[1]).toDate();
                            } else {
