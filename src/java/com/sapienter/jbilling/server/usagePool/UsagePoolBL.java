...
public class UsagePoolBL {
	...
	private static final Logger LOG = Logger.getLogger(UsagePoolBL.class);
    ...
    public void validateUsagePoolWS(UsagePoolWS usagePoolWs) throws SessionInternalError {
    	// check if the usage pool quantity is zero or negative, if yes then return error
    	if (NumberUtils.isNumber(usagePoolWs.getQuantity()) && 
    		new BigDecimal(usagePoolWs.getQuantity()).doubleValue() <= 0.00) {
    	...
    	if (null != usagePoolWs && null != usagePoolWs.getConsumptionActions() && !usagePoolWs.getConsumptionActions().isEmpty()) {
    		for(UsagePoolConsumptionActionWS consumptionAction : usagePoolWs.getConsumptionActions()) {
    			...
    			if (null != percentage &&
					(!NumberUtils.isNumber(percentage) ||
						AttributeUtils.parseDecimal(percentage).compareTo(BigDecimal.ZERO) <= 0 ||
						AttributeUtils.parseDecimal(percentage).compareTo(new BigDecimal(100)) > 0)) {
    				throw new SessionInternalError("Usage Pool Consumption % must be integer or fraction value which is greater than zero and less than or equal to 100",
    	        			new String[]{ "UsagePoolWS,attribute_key,usagePool.error.attribute.key.must.be.positive.integer" });
    			} else if (null != percentage &&
    						(NumberUtils.isNumber(percentage) && AttributeUtils.parseDecimal(percentage).compareTo(BigDecimal.ZERO) > 0)) {
    				if (null == type || (null != type && type.isEmpty())) {
                    	...
                    	if (null == productId ||
                                !NumberUtils.isNumber(productId) ||
                                AttributeUtils.parseDecimal(productId).compareTo(BigDecimal.ZERO) < 0) {
    ...
    private void checkRequirdUsagePoolName(UsagePoolWS usagePoolWs) throws SessionInternalError {
        ...
        //check if english name is present
        boolean hasEnglishName = false;
    	for (InternationalDescriptionWS name : names) {
    		 if (name.getContent() != null && !name.getContent().trim().equals("")) {
    			 if (name.getLanguageId() == LanguageDTO.ENGLISH_LANGUAGE_ID && !name.isDeleted()) {
    				 hasEnglishName = true;
    	            }
       		 }
    	}
    ...
    private UsagePoolDTO getCreateOrUpdateDTO(UsagePoolWS usagePoolWs) {
    	...
    	if (usagePoolWs.getId() == 0) {
			usagePoolDto.setCreatedDate(new Date());
		}
     ...
     * @throws SessionInternalError
     */
    public boolean delete(Integer usagePoolId) throws SessionInternalError {
    	...
    	try {
	     ...
	     if (isDeletable != null) {
    		...
    		LOG.debug("Exception while deleting usage pool: " + e);
    ...
    public Boolean isDeletable(){
    	if (this.usagePool != null) {
    		List<PlanDTO> plans= 
    				new PlanDAS().findPlansByUsagePoolId(this.usagePool.getId());
    		return plans == null || (plans != null && plans.isEmpty());
    	}
