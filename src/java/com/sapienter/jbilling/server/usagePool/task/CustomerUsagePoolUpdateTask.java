		...
		if (event instanceof NewOrderEvent) {
			...
			if (deletedOrder.hasFreeUsagePools()) {
				...
				for (Entry<Integer, BigDecimal> freeUsagePoolEntry : deletedOrder.getFreeUsagePoolsMap().entrySet()) {
					...
					if (customerPoolUpdateQuantity.compareTo(customerUsagePoolDto.getUsagePool().getQuantity()) > 0) {
						
						BigDecimal newPoolQuantity = customerUsagePoolDto.getUsagePool().getQuantity();
