 ...
 */
public class SendEnrollmentTask extends AbstractCronTask {
    ...
    private static final FormatLogger LOG = new FormatLogger(Logger.getLogger(SendEnrollmentTask.class));
...
//            Set values in local variable.
        ...
        try {
            ...
            String targetPath = FileConstants.COMMUNICATION_OUTBOUND_PATH;
...
//            successFile.setEdiFileStatusWS(successStatusWS);
            LOG.debug("File ws with new Status:  " + successFile.getEdiFileStatusWS().getName());
             webServicesSessionSpringBean.updateEDIStatus(successFile, successStatusWS);
//            LOG.debug("After Saving edi file with id" + successFileId);
//            LOG.debug("After Saving edi file successFile.getFileStatus().getId() " + successFile.getEdiFileStatusWS().getId());
        }
    }
