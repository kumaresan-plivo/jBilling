...
public class BrokerResponseManagerTask extends PluggableTask implements IInternalEventsTask {
    ...
    private final String header = "\"Broker Id\",\"LDC\",\"Account Number\",\"Response Code\",\"Reason\",\"Timestamp\"";
    ...
    public void process(Event event) throws PluggableTaskException {
        if (event instanceof EnrollmentEvent && ((EnrollmentEvent) event).getCustomerEnrollment().isBulkEnrollment()) {
            ...
            this.generateResponseFile(enrollmentEvent.getEntityId(), enrollmentEvent.getCustomerEnrollment().getBrokerId(), customerEnrollmentResponse);
        }
    }
    ...
    private CustomerEnrollmentResponse generateCustomerEnrollmentResponse(EnrollmentEvent enrollmentEvent, CustomerEnrollmentResponse.Code responseCode) {
        ...
        customerEnrollmentResponse.setBrokerId(enrollmentEvent.getCustomerEnrollment().getBrokerId());
        ...
        customerEnrollmentResponse.setTimestamp(new Date());

        return customerEnrollmentResponse;
    }

    private boolean generateResponseFile(Integer entityId, String brokerId, CustomerEnrollmentResponse customerEnrollmentResponse) {
        try {
            File file = Paths.get(this.getOutboundFolderPath(entityId), brokerId + "-" + dateFormat.format(customerEnrollmentResponse.getTimestamp()) + ".csv").toFile();
