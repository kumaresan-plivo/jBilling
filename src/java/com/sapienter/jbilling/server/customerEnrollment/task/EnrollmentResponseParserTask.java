...
//    public static final Long TRANSACTION_SET = new Long(814);
    ...
    private EDIFileWS ediFileWS = null;
...
//        Set transaction type from suffix.
    ...
    void processEnrollmentResponseFile() throws Exception {
        ...
        parseRecords(maidenRecords, ediFileWS.getEDIFileRecordWSes());
...
//        Create customer from enrollment from here.
        ...
        try {
            ediTransactionBean.isUniqueKeyExistForFile(companyId, ediTypeId, ediFileWS.getId(), FileConstants.TRANS_REF_NR, transferNRValue, TransactionType.INBOUND);
...
//        Get customer enrollment id here.
        customerEnrollmentId = getCustomerEnrollmentId(ediFileWS);
        ...
        String responseAccountNumber = findField(EnrollmentResponseParser.ACT, EnrollmentResponseParserField.UTILITY_CUST_ACCT_NR.toString());

        LOG.debug("Enrolled Account Number: "+enrollmentAccountNumber);
        LOG.debug("Response Account Number: "+responseAccountNumber);

        if(!enrollmentAccountNumber.toUpperCase().equals(responseAccountNumber)){
    ...
    private void createCustomerAndOrder() {
        if (outboundFileWS != null && outboundFileWS.getEdiFileStatusWS().getName().equals(SUCCESS_STATUS)) {
            try {
                ...
                //TODO is meter cycle is mandatory
                ...
                BigDecimal planPrice=item.getPrice(new Date(), customerEnrollmentWS.getEntityId()).getRate();
                ...
                if (planDTO.getMetaField(FileConstants.BILLING_MODEL) != null) {
                    ...
                    if (FileConstants.BILLING_MODEL_RATE_READY.equals(planDTO.getMetaField(FileConstants.BILLING_MODEL).getValue())) {
                        String rate = findRate(ediFileWS);
                        ediFileData.put("Rate", rate);
                    }
                } else {
                ...
                ediTransactionBean.createCustomerAndOrder(customerEnrollmentWS, ediFileData);
...
//                Set Enrollment status after creating customer and it's order successfully.
     ...
     * Then Rate change date of customer should be equal to “CUST_ENROLL_AGREE_DT"
     */
    private String getRateChangeDate(int itemId, Integer meterCycle) {
        ...
        if (ediTransactionBean.hasPlanSendRateChangeDaily(planDTO)) {
            ...
            Date CustEnrollAgreeDT = (Date) customerEnrollmentWS.getMetaFieldValue(FileConstants.CUST_ENROLL_AGREE_DT);
