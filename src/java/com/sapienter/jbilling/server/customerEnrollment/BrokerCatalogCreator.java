...
public class BrokerCatalogCreator {
    ...
    private BrokerCatalogResponse generateBrokerCatalogResponse(Integer companyId, PlanDTO plan, String catalogVersion) {
        ...
        if (!plan.getPlanItems().isEmpty()) {
            ...
            if (productType.equals(ProductType.FP.name())) {
                ...
                String rateParam = planPriceModel.getAttributes().get(TeaserPricingStrategy.PARAM_RATE_PREFIX + "0");
            ...
            //Discount Period
            Integer discountPeriod = null;
            if (productType.equals(ProductType.TP.name())) {
                String cycleParam = planPriceModel.getAttributes().get(TeaserPricingStrategy.PARAM_CYCLE_PREFIX + "0");
                discountPeriod = StringUtils.isNotEmpty(cycleParam) ? Integer.valueOf(cycleParam) : null;
            }
    ...
    private File generateResponseFile(List<BrokerCatalogResponse> brokerCatalogResponses, Integer companyId, String catalogVersion) {
        ...
        try {
            file = Paths.get(this.getCatalogFolderPath(companyId), catalogVersion + "-" + dateFormat.format(new Date()) + ".csv").toFile();
