...
environments {
    ...
    development {
        grails.serverURL = "http://localhost:8080/jbilling"
    }
}
...
log4j = {
    appenders {
        ...
        appender new org.apache.log4j.DailyRollingFileAppender(name: 'jbilling', file: 'logs/jbilling.log',
                datePattern: '\'_\'yyyy-MM-dd', layout:pattern(conversionPattern: '%d %-5r %-5p [%c] (%t:%x) %m%n'))
	appender new org.apache.log4j.DailyRollingFileAppender(name: 'sql', file: 'logs/sql.log',
                datePattern: '\'_\'yyyy-MM-dd', layout:pattern(conversionPattern: '%d %-5r %-5p [%c] (%t:%x) %m%n'))
        rollingFile name: "stacktrace", maxFileSize: '5MB', maxBackupIndex:10, file: "logs/jbilling-stacktrace.log"

        appender new org.apache.log4j.DailyRollingFileAppender(name: 'diameter', file: 'logs/diameter.log',
                datePattern: '\'_\'yyyy-MM-dd', layout:pattern(conversionPattern: '%d %-5r %-5p [%c] (%t:%x) %m%n'))

        appender new org.apache.log4j.DailyRollingFileAppender(name: 'jbilling-debug', file: 'logs/jbilling-debug.log',
                datePattern: '\'_\'yyyy-MM-dd', layout:pattern(conversionPattern: '%d %-5r %-5p [%c] (%t:%x) %m%n'))
    }
...
//      org.hibernate               Log everything. This is a lot of information but it is useful for troubleshooting
    ...
    // debug additivity: false, sql: "org.hibernate.SQL"
     ...
     debug additivity: false, 'jbilling-debug': "org.springframework.core.io.support.PropertiesLoaderSupport"
     debug additivity: false, 'jbilling-debug': "org.springframework"
}
...
grails.resources.modules = {
    ...
    wizard{
        resource url: '/js/enrollment-wizard.js', disposition: "defer"
...
grails.plugin.springsecurity.controllerAnnotations.staticRules = [
        ...
        '/j_spring_security_switch_user': ["hasAnyRole('ROLE_SUPER_USER','ROLE_SYSTEM_ADMIN')",'IS_AUTHENTICATED_FULLY']
]
