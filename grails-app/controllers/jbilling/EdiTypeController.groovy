...
class EdiTypeController {
    ...
    def edit() {
        ...
        if (request.isXhr()) {
            ...
            chain action: "list", model: [ediType: ediTypeWS]
            return
        }
    ...
    def save() {
        ...
        ediTypeWS.setCreateDatetime(new Date())
        if (global) {
        ...
        ediTypeWS.setEntityId(session["company_id"])
...
//                If directory not exist, create it first. TODO: This code can be moved in prepare test.
            ...
            ediStatusName.eachWithIndex { String status, Integer index ->
                if (status) {
                    EDIFileStatusWS ediFileStatusWS = new EDIFileStatusWS(createDatetime: new Date(), name: status)
            ...
            redirect(action: "list", params: [id: ediTypeId])
        } catch (SessionInternalError e) {
            viewUtils.resolveException(flash, session.locale, e)
            chain action: "list", model: [ediType: ediTypeWS], params: [id:ediTypeWS?.id]
            return
        }
    }
    ...
    def download() {
        Integer id = params.int("id")
        EDITypeWS ediTypeWS = webServicesSession.getEDIType(id)

        def file = new File(Util.getSysProp("base_dir") + File.separator + FileConstants.EDI_TYPE_FORMAT_PATH + File.separator + "${ediTypeWS.path + FileConstants.HYPHEN_SEPARATOR + ediTypeWS.getEntityId()}.xml")
        ...
        if (file.exists()) {
            ...
            chain action: "list", params: [id:ediTypeWS?.id]
        }
    }

    def uploadOutboundFile(){
        Integer ediTypeId=params.int('ediTypeId')
        EDITypeDTO ediTypeDTO=EDITypeDTO.get(ediTypeId)
        try{
            def eventFile = request.getFile("file")

            if(eventFile?.originalFilename){
                    def fileDir = new File(FileConstants.COMMUNICATION_INBOUND_PATH);
                    if (!eventFile.isEmpty()) {
                        new File(fileDir.toString() + "/").mkdirs()
                        eventFile.transferTo(new File("${fileDir.toString()}/${eventFile?.originalFilename?.replace('.txt','')}"))
                    }
