...
class SignupController {
	...
	@Secured(["ROLE_SYSTEM_ADMIN", "ROLE_SUPER_USER"])
	def reseller (){
		render view: 'index'
	}
    ...
    def createUser(language, currency, company) {
        ...
        def rootRole = new RoleDAS().findByRoleTypeIdAndCompanyId(
			Constants.TYPE_SYSTEM_ADMIN, company.id)
    ...
    def copyCompany() {
        ...
        if (entityId) {
            try {
                UserWS userWS = webServicesSession.copyCompany(childCompanyTemplateName, entityId, importEntities, isCompanyChild, copyProducts, copyPlans)
                flash.message = g.message(code: 'copy.company.create.label', args: [userWS.userName, userWS.password, CompanyDTO.get(userWS.entityId).description, userWS.entityId])
            }
            catch (SessionInternalError e) {
                viewUtils.resolveException(flash, session?.locale ?: new Locale("en"), e)
            }
