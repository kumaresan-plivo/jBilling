...
class CustomerInspectorController {
	...
	def inspect () {
		...
		def customerUsagePools = webServicesSession.getCustomerUsagePools(user?.customer?.id)
        ...
        String xml = session['configurationFile'] ?: CompanyDTO.get(session['company_id']).getCustomerInformationDesign()
    ...
    def productPrices () {
        ...
        securityValidator.validateCompany(product?.entity?.id, Validator.Type.VIEW)
    ...
    def saveCustomerPrice () {
        ...
        try {
            ...
            render view: 'editCustomerPrice', model: [ price: price, product: product, user: user, currencies: retrieveCurrencies(true), priceExpiryDate: params.priceExpiryDate ]
            return
        } catch (Exception e) {
            log.error e.getMessage()
