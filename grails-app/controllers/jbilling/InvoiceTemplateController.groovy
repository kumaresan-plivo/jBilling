...
class InvoiceTemplateController {
    ...
    def uploadJson () {
        ...
        try {
            ...
            if (params.name) {
                ...
                if (InvoiceTemplateDTO.findByName(params.name) != null) {
                    render(status: HttpStatus.SC_INTERNAL_SERVER_ERROR, text: message(code: "invoiceTemplate.error.name.duplicate"))
                    return
                }

                newDesign.setName(params.name.toString())
            } else {
                render(status: HttpStatus.SC_INTERNAL_SERVER_ERROR, text: message(code: "invoiceTemplate.error.name.empty"))
                return
            }
            ...
            if (params.json) {
                ...
                render(status: HttpStatus.SC_INTERNAL_SERVER_ERROR, text: message(code: "invoiceTemplate.error.emptyJson"))
                return
            }
        } catch (Exception ex) {
            // The provided JSON is invalid so show an error.
            render(status: HttpStatus.SC_INTERNAL_SERVER_ERROR, text: message(code: "invoiceTemplate.error.invalidJson"))
            return
        }
    ...
    def showVersion () {
        ...
        render(view: 'list', model: [selectedTemplateVersion:selectedTemplateVersion, invoiceTemplateVersions: versionDTOs,
