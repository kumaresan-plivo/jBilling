...
class RateCardController {
    ...
    def edit (){
        ...
        breadcrumbService.addBreadcrumb(controllerName, actionName, params.name ? 'update' : 'create', params.int('id'))
        ...
        if (params.rates?.getContentType().toString().contains('text/csv') ||
	        ...
	        try {
	            ...
	            if (!rateCard.id) {
					...
					rateCardDTO.setChildCompanies(convertToCompanyDTO(rateCard.getChildCompanies()));
    ...
    def Set<CompanyDTO> convertToCompanyDTO(entities){
    ...
    def downloadExampleCSV() {
        ...
        DownloadHelper.setResponseHeader(response, "${file.name}.csv")
        render text: file.text, contentType: "text/csv"
    }
}
