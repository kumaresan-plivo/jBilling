    ...
    def list () {
        ...
        if (usingJQGrid){
            if (params.applyFilter || params.partial) {
                render template: 'invoicesTemplate', model: [currencies: retrieveCurrencies(), filters: filters, contactFieldTypes: contactFieldTypes]
            }else {
                render view: 'list', model: [currencies: retrieveCurrencies(), filters: filters, contactFieldTypes: contactFieldTypes, selected: selected]
            }
            return
        }
        ...
        if (params.applyFilter || params.partial) {
            render template: 'invoicesTemplate', model: [invoices: invoices, filters: filters, selected: selected, currencies: retrieveCurrencies(), contactFieldTypes: contactFieldTypes]
        } else {
            def lines = null
            if(selected){
...
//                                            addToCriteria(Restrictions.eq("fieldValue.value", fieldValue.getValue()))
    ...
    def linehistory (){
        ...
        securityValidator.validateUserAndCompany(UserBL.getWS(new UserDTOEx(line?.invoice?.baseUser)), Validator.Type.VIEW)
