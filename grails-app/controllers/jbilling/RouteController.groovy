...
class RouteController {
    ...
    def list (){
        ...
        def edit = chainModel?.selected ? true : false
    ...
    def saveMatchingField (){
        ...
        try {
            ...
            viewUtils.resolveException(flash, session.locale, e)
            chain action: 'list', model: [matchFieldWS: matchFieldWS, selectMatchingField: selectMatchingField]
            return
        }
    ...
    def save (){
        ...
        // save uploaded file
        CommonsMultipartFile routes = request.getFile("routes")
        def temp = null
        ...
        try {
            if (params.routes?.getContentType().toString().contains('text/csv') ||
                    params.routes?.getOriginalFilename().toString().endsWith('.csv')
                    || (route.id)) {
    ...
    def search (){
        ...
        render view: 'searchRouteRecordsGrid', model: [
                route: route,
