...
class NotificationsController {
    ...
    def saveNotification () {
        ...
        try {
            ...
            redirect(action: 'failSave', params: params)
            return
        } catch (SessionInternalError e) {
            log.error "Error: " + e.getMessage()
            flash.error = "error.illegal.modification"
    ...
    def saveAction(params) {
        ...
        if(params.includeAttachment.equals("on") && !new File(com.sapienter.jbilling.common.Util.getSysProp("base_dir")
                + "designs" + File.separator + params.attachmentDesign + ".jasper").exists()) {
    ...
    def failSave(params){
        NotificationMessageTypeDTO typeDto = params.get('_id') ? NotificationMessageTypeDTO.findById(params.get('_id')) : null
        NotificationMessageDTO dto = null
        for (NotificationMessageDTO messageDTO : typeDto?.getNotificationMessages()) {
        ...
        flash.error = 'notification.enter.design.not.exists'
        render view: 'edit', model: [dto: dto,
                                     messageTypeId: params.get('_id'),
    ...
    def Set<NotificationMessageSectionDTO> bindMessageSection(NotificationMessageDTO dto, GrailsParameterMap params){
