...
class PartnerController {
    ...
    */
        ...
        try {
            ...
            if(PartnerCommissionType.CUSTOMER.name().equals(partner.getCommissionType())) {
                ...
                try {
                    ...
                    error = new SessionInternalError(e)
                    error.setErrorMessages(['partner.error.commission.value.rate.not.decimal'].toArray(new String[0]))
                ...
                commissionValues += new PartnerCommissionValueWS(0, value)
                    ...
                    if(it.key.isNumber()) {
                        ...
                        try {
                            ...
                            if(!error) {
                                error = new SessionInternalError(e)
                                error.setErrorMessages(['partner.error.commission.value.days.not.integer'].toArray(new String[0]))
                            }
                            days = 0
                        }
                        BigDecimal value = null
    ...
    private void securityCheck(PartnerDTO partner){
        ...
        if(throwException){
            throw new SecurityException("Unauthorized access by caller '" + loggedInUser.getUserName() + "' (id " + loggedInUser.getId() + ")");
        }
    }
