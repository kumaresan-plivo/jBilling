...
class ProductController {
    ...
    def list () {
		...
		if (category && !((Boolean) companyService.isAvailable(category.global, category.entity?.id, category.entities*.id))) {
			category= null
...
//                                            addToCriteria(Restrictions.eq("fieldValue.value", fieldValue.getValue()))
    ...
    def show () {
        ...
        if (params.template) {
            ...
            render view: 'list', model: [ categories: categories, products: products, selectedProduct: product,
                    assetManagementPossible: assetManagementPossible, selectedCategory: productCategory, filters: filters]
        }
    }
     ...
     */
    @Secured(["hasAnyRole('ASSET_130', 'ASSET_131')"])
    def editAsset () {
        def asset = params.id ? AssetDTO.get(params.id) : new AssetDTO()

        securityValidator.validateCompany(asset?.entity?.id, Validator.Type.VIEW)
    ...
    def saveAsset () {
		...
		if (!isRoot) {
			...
			asset.entities = new ArrayList<Integer>(0);
				...
				if(asset.getEntities() == null || asset.getEntities().size() == 0) {
                    ...
                    flash.args = [ params.id  as String]
                    redirect (controller: 'product', action: 'editAsset', params: [id:null, partial: true, add: true, prodId: asset.itemId])
                    return
				}
		}
        ...
        try {
            if (asset.id) {
                ...
                if (SpringSecurityUtils.ifAllGranted("ASSET_130")) {
                ...
                if (SpringSecurityUtils.ifAllGranted("ASSET_131")) {
    ...
    def loadAssetGroupFilters () {
        ...
        render template:  'groupSearchFilter', model:  [assetStatuses: itemTypeDTO?.assetStatuses, products: itemTypeDTO?.items?.findAll { it.deleted == 0 && !it.isPlan() }, metaFields: itemTypeDTO?.assetMetaFields]
    }
    ...
    def editCategory () {
        ...
        securityValidator.validateUserAndCompany(ItemTypeBL.toWS(category), Validator.Type.EDIT)
    ...
    def saveDate () {
        ...
        boolean shouldSetEntityOnPrice = product.getPriceModelCompanyId() == null &&
                new CompanyDAS().find(webServicesSession.getCallerCompanyId()).isInvoiceAsReseller();
    ...
    */
    ...
    def history (){
        ...
        def price = product.getPrice(new Date())
