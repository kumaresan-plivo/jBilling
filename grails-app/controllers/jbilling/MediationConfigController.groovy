...
class MediationConfigController {
    ...
    def list (){
        ...
        if (params.applyFilter) {
            ...
            if(params.id){
                selected=MediationConfiguration.get(params.id)
	...
	def save (){
        ...
        chain action: 'list', params: [ id: ws?.id ], model: [mediationConfiguration:!isSaved ? ws:null]
	}
        ...
        try {
			if(doRun) {
				...
				if (fileInjectionEnabled) {
                    ...
                    mediationService.launchMediation(entityId, configId, mdConfig.getMediationJobLauncher(), temp)
				} else {
					if (recordsString) {
                        mediationService.processCdr(entityId, configId, mdConfig.getMediationJobLauncher(), recordsString)
	                } else {
                        doRun = false
    ...
    def showAllMediationErrorOfMediationConfig(){
        ...
        def endDate= params.endDate ? new Date().parse(message(code: 'date.format'), params.endDate) : null
        def statusId = 3
        def entityId = session['company_id']
        int CDR_PAGE_SIZE = 20
        def offset=params.offset?:null
        if (params.first=='true'){
            offset=null
        }
        //TODO MODULARIZATION: retrieve error records with this fields
        def mediationErrorRecords = null; //mediationSession.getMediationErrorRecordsByMediationConfiguration(mediationConfiguration, entityId, statusId, offset, CDR_PAGE_SIZE, startDate, endDate);

        if (mediationErrorRecords) {
            offset= getMediationErrorKey(mediationErrorRecords.last(), mediationConfiguration)
        }
        def record
        if (mediationErrorRecords) {
        ...
        render view: 'errors', model: [records: mediationErrorRecords,mediationConfiguration:mediationConfiguration, record: record, errorCodes: errorCodes, offset:offset, startDate:params.startDate, endDate:params.endDate]
    }
    ...
    private getMediationErrorKey(JbillingMediationErrorRecord record, MediationConfiguration mediationConfiguration) {
        ...
        keyBuilder.append(record.getMediationProcessId()).append("-");
        keyBuilder.append(record.getKey());
