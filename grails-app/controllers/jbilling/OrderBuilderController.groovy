    ...
    def editFlow = {//do not change this to method. Webflow will not be able to find it.
				...
				conversation.order?.orderLines.each {
					...
					conversation.discountableItems << new DiscountableItemWS(it.itemId, 
															...
															it.description + 
																...
																(product.getPrice(new Date(), session['company_id'])?.type != PriceModelStrategy.LINE_PERCENTAGE ?
																	" - Rate=" + 
									...
									if (it.bundle?.quantity && conversation.order?.orderLines) {
										conversation.discountableItems << new DiscountableItemWS( it.id, 
												...
												(it.plan.description?:it.plan.item.description) + " - " + 
													...
													(it.item?.getPrice(new Date(), session['companyId'])?.type != PriceModelStrategy.LINE_PERCENTAGE ?
            ...
            action {
                ...
                conversation.plans?.each{it.getEntities().size(); it.getDefaultPrices().size(); it.getItemTypes()?.size();
                    for (def planItem : it.plans.asList().first()?.planItems) {
                        def item = planItem?.item.assetManagementEnabled
                        def bundle = planItem?.bundle?.quantity
                    }
                }
                conversation.maxPlansShown = params.max
            }
        ...
        addOrderLine {
            action {
				...
				if(product.getPrice(new Date(), session['company_id']).getType() == PriceModelStrategy.LINE_PERCENTAGE) {
                ...
                if (!conversation.plan) {
                    PriceModelDTO priceModelDto = product.getPrice(conversation.pricingDate, session['company_id'])
                    if (priceModelDto) {
                        if (priceModelDto.type == PriceModelStrategy.TIERED)
                        	line.price = new BigDecimal(priceModelDto.attributes.get("0"))
						else if (priceModelDto.type == PriceModelStrategy.VOLUME_PRICING)
							line.price = new BigDecimal(priceModelDto.attributes.get("0"))
                        else
                        	line.price = priceModelDto.rate
                    ...
                    if (!product.percentage) {
                        def priceModelDto = product.getPrice(conversation.order.activeSince ?: conversation.order.createDate ?: new Date(), session['company_id'])
            ...
            action {
                ...
                if (change.orderLineId < 0 && params["change-${changeId}.itemIds"]) {
                    ...
                    itemIds.each {
                        ...
                        orderChangePlanItem.metaFields = MetaFieldBindHelper.bindMetaFields(planItemProduct?.orderLineMetaFields?.findAll { it.getDataType() != DataType.STATIC_TEXT }, params)
        ...
        saveOrder {
            action {
                ...
                def idsMap = [:]
                try {
                    // verify order does not contain more than one plan in it
                    def plans = conversation.plans
                    def hasMoreThaOnePlan = order.orderLines.findAll{ l -> plans.find{ p -> p.id == l.itemId && l.deleted == 0 }}.size > 1
                    if (hasMoreThaOnePlan) {
                        //An order can not have more than one active plan
                        throw new SessionInternalError("Cannot subscribe to more than one plan at a time. To add another plan, create a new order.");
                    }
        ...
        if (line.id <= 0 && planDto && planDto.getPlanItems()) {
            ...
            for (PlanItemDTO planItemDto : planItemDtos) {
                ...
                if (editablePlan) {
                    ...
                    PriceModelDTO priceModelDto = PriceModelBL.getPriceForDate(planItemDto.models, pricingDate ?: new Date())
                    if (priceModelDto) {
                    ...
                    if (product.assetManagementEnabled == 1 || product.orderLineMetaFields || addNonBundled ||
                        ...
                        orderChangePlanItem.description = planItemDto.getItem().getDescription()
