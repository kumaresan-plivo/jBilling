        ...
        <table id="paymentTransfer" cellspacing="0" cellpadding="0">
            ...
            <g:each var="paymentTransfer" in="${paymentTransfers}">
                ...
                <tr id="payment-${paymentTransfer.id}" class="${selected?.id == paymentTransfer.id ? 'active' : ''}">
                    ...
                    <td class="medium">
                        <g:remoteLink class="cell" action="show" id="${paymentTransfer.id}" before="register(this);" onSuccess="render(data, next);">
                            <span><g:formatDate date="${paymentTransfer.createDatetime}" formatName="date.pretty.format"/></span>
                        </g:remoteLink>
                    </td>
                    <td class="small">
                        <g:remoteLink class="cell" action="show" id="${paymentTransfer.id}" before="register(this);" onSuccess="render(data, next);">
...
<div class="pager-box">
    ...
    <div class="row">
        <util:remotePaginate controller="paymentTransfer" action="list" params="${sortableParams(params: [partial: true])}" total="${paymentTransfer?.totalCount ?: 0}" update="column1"/>
    </div>
</div>