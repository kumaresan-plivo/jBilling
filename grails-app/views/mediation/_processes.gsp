					...
					<tr id="mediation-${proc.id}" class="${proc?.id == processId ? 'active' : ''}">
						<td>
							<g:remoteLink breadcrumb="id" class="cell" action="show" id="${proc.id}" params="['template': 'show']" before="register(this);" onSuccess="render(data, next);">
                                <g:formatDate date="${proc.startDate}" formatName="date.timeSecsAMPM.format"/>
                            </g:remoteLink>
						</td>
                        <td>
                            <g:remoteLink breadcrumb="id" class="cell" action="show" id="${proc.id}" params="['template': 'show']" before="register(this);" onSuccess="render(data, next);">
                                <g:formatDate date="${proc.endDate}" formatName="date.timeSecsAMPM.format"/>
                            </g:remoteLink>
                        </td>
						<td>
                            <g:remoteLink breadcrumb="id" class="cell" action="show" id="${proc.id}" params="['template': 'show']" before="register(this);" onSuccess="render(data, next);">
...
<div class="pager-box">
    ...
    <div class="row">
        <util:remotePaginate controller="mediation" action="index" params="${sortableParams(params: [partial: true])}" total="${processes?.recordsProcessed ?: 0}" update="column1"/>
    </div>
</div>
