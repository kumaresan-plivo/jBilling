...
<g:each var="field" in="${availableFields?.sort{ it.displayOrder }}">
    <g:set var="metaFieldTemplateName" value="${WordUtils.uncapitalize(WordUtils.capitalizeFully(field.getDataType().name(), ['_'] as char[]).replaceAll('_',''))}"/>
    <g:if test="${!field.disabled}">
        ...
        <g:render id="dataTypeTemplate" template="/metaFields/dataType/${metaFieldTemplateName}" model="[
