...
<%@ page import="com.sapienter.jbilling.common.Constants; com.sapienter.jbilling.server.item.PlanItemBL; com.sapienter.jbilling.server.metafields.db.MetaField; com.sapienter.jbilling.server.item.db.AssetDTO; com.sapienter.jbilling.server.pricing.PriceModelBL; org.apache.commons.lang.WordUtils; com.sapienter.jbilling.server.item.db.PlanItemBundleDTO; com.sapienter.jbilling.server.item.db.ItemDTO" %>
...
<tr id="edit_change_details_${changeId}" class="change_details"
    ...
    <td colspan="3">
        <g:formRemote name="change-${changeId}-update-form" url="[action: 'edit']" update="ui-tabs-edit-changes"
            ...
            <g:if test="${currentPlan && orderChange.change.orderLineId < 0 && currentPlan.editable == 0}">
                <ul class="plan-order-changes-list">
                    <g:set var="displayedOrderChangePlanItems" value="${[]}"/>
                    <g:each var="planItem" in="${PlanItemBL.collectPlanItemsIncludingNested(currentPlan)}">
                        <g:set var="orderChangePlanItem" value="${orderChange.change.orderChangePlanItems?.find { it.itemId == planItem.item.id && !displayedOrderChangePlanItems.contains(it) }}"/>
                        <g:if test="${orderChangePlanItem}">
                            <g:set var="putToDisplayed" value="${displayedOrderChangePlanItems << orderChangePlanItem}"/>
                            <g:render template="editChangePlanItem" model="[changeObj: orderChange.change, user: user, planItem: planItem, changePlanItem: orderChangePlanItem]"/>
                        </g:if>
                    </g:each>
                </ul>
            </g:if>
