...
<g:set var="itemId" value="${planItem?.item?.id}"/>
<g:set var="planItemPrice" value="${PriceModelBL.getPriceForDate(planItem.models,pricingDate)}"/>
<g:set var="planItemProduct" value="${planItem.item}"/>
...
<g:set var="currencySymbol" value="${planItemPrice?.currency.symbol}" />
<g:if test="${planItem?.bundle?.quantity || showZeroQuantityBundle || planItemProduct?.assetManagementEnabled || planItemProduct?.orderLineMetaFields}">
    <li id="edit_change_header_${changeId}_${itemId}" class="line active" onclick="showEditChangeDetails(this);">
        ...
        <span class="sub-total">
            <g:set var="changePlanItemSubTotal" value="${( planItemPrice?.rate ?: BigDecimal.ZERO ).multiply(planItem?.bundle?.quantity ?: BigDecimal.ZERO)}"/>
            <g:set var="subTotal"
        ...
        <span class="qty-price">
            <g:set var="quantity" value="${formatNumber(number: planItem?.bundle?.quantity ?: BigDecimal.ZERO, formatName: quantityNumberFormat)}"/>
            <g:set var="price" value="${formatNumber(number: planItemPrice?.rate ?: BigDecimal.ZERO, type: 'currency', currencySymbol: StringEscapeUtils.unescapeHtml(currencySymbol), formatName: 'price.format')}"/>
    ...
    <div class="box">
        <div class="form-columns">
            ...
            <g:applyLayout name="form/text">
                ...
                ${formatNumber(number: planItem?.bundle?.quantity, formatName: quantityNumberFormat)}
            </g:applyLayout>

            <sec:ifAllGranted roles="ORDER_26">
                <g:applyLayout name="form/text">
               		<g:set var="price" value="${planItemProduct.getPrice(new Date(), session['company_id'])}"/>
