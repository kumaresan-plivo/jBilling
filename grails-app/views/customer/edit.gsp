...
<%@ page import="com.sapienter.jbilling.server.invoice.InvoiceTemplateDTO; com.sapienter.jbilling.server.user.db.UserDAS" %>
                    ...
                    <div class="column">
                        ...
                        <g:if test="${user?.partnerIds || !partnerId}">
                            <g:applyLayout name="form/input">
                                ...
                                <g:textField class="field" name="user.partnerIdList" value="${user?.partnerIds?.join(',') ?: partnerId}"/>
                            </g:applyLayout>
                        </g:if>
                        <g:else>
                            <g:applyLayout name="form/text">
                    ...
                    %{-- Allow the user to add linked user codes --}%
					   ...
					   <g:render id="subscriptionTemplate" template="/customer/subscription/${templateName}" model="[mainSubscription: mainSubscription]" />
						...
						<sec:ifNotGranted roles="CUSTOMER_121">
							...
							<div class="select5">
							<content tag="label">
							<g:message code="prompt.main.subscription"/></content>
							<content tag="label.for">mainSubscription.periodId</content>
							<g:select id="mainSubscription" from="${orderPeriods}"
							optionKey="id" optionValue="${{it.getDescription(session['language_id'])}}"
							name="mainSubscription.periodId"
                        ...
                        <g:preferenceEquals preferenceId="${Constants.PREFERENCE_ITG_INVOICE_NOTIFICATION}" value="1">
                            <g:applyLayout name="form/select">
                                ...
                                <g:select name="user.invoiceTemplateId"
                                          ...
                                          value="${user?.invoiceTemplateId}" optionKey="id" optionValue="name"/>
                            </g:applyLayout>
                        </g:preferenceEquals>
