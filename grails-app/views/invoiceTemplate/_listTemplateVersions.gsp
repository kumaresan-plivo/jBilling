...
<div class="table-box">
    <div class="table-scroll">
        <table id="templateVersions" cellspacing="0" cellpadding="0">
            ...
            <g:each in="${invoiceTemplateVersions}" var="templateVersion">
                <tr class="${templateVersion.id == selectedTemplateVersion?.id ? 'active' : ''}">
                    ...
                    <td>
                        <g:remoteLink class="cell double" action="showVersion" id="${templateVersion.id}"
                                      before="register(this);" onSuccess="render(data, next);">
        ...
        <div class="pager-box">
            ...
            <div class="row">
                <util:remotePaginate  controller="invoiceTemplate" action="listVersions" id="${selectedTemplate?.id}" total="${versionsTotalCount}" onSuccess="closePanel(\'#column2\');" update="column1"/>
            </div>
        </div>
    </div>
    <div class="btn-hold">
        <div class="btn-box">
            <div class="row">
                <sec:ifAllGranted roles="INVOICE_TEMPLATES_1804">
                    <a onclick="showConfirm('deleteTemplate-${selectedTemplate?.id}');" class="submit delete"><span><g:message code="button.delete"/></span></a>
                </sec:ifAllGranted>
            </div>
        </div>
    </div>
