...
<div class="form-edit">
    ...
    <div class="form-hold">
                    ...
                    <g:if test="${queries}">
                        <div id="queryList" >
                            <g:applyLayout name="form/select">
                                ...
                                <g:select name="queryId" from="${queries}" optionKey="id" optionValue="name" onchange="checkIfCanDeleteQuery();" onkeypress="if (event.keyCode == 13) execQuery();"/><a class="submit4" onclick="execQuery();"><span class="submit4span">▷</span></a><a id="delQueryBtn" class="submit4" onclick="deleteQueryEntry();"><span class="submit4span">✖</span></a>
                            </g:applyLayout>
                        </div>
                    </g:if>
                ...
                <g:form name="nested-search-form" action="nestedSearch">
                    <fieldset>
                        ...
                        <g:each in="${nestedTables}" var="nestedTable" status="depth">
                            <div class="row">
                                <label for=""><g:message
                                        code="dataTable.nested.search.depth.label"/>&nbsp;${depth + 1}</label>
                                <span class="normal">
                                    ...
                                    <span class="strong">&nbsp;&nbsp;<g:message
                                            code="dataTable.nested.search.columns.label"/>:</span>
                                    ${nestedColumns[depth]}
                                    <g:hiddenField name="nestedColumns[${depth}]" value="${nestedColumns[depth]}"/>
                                </span>
                            </div>
                            ...
                            <div class="row">
                                ...
                                <span class="normal">
                                    <span class="strong"><g:message
                                            code="dataTable.nested.search.values.label"/>:</span> ${nestedValues[depth]?.decodeHTML()}
                                <g:hiddenField name="nestedValues[${depth}]" value="${nestedValues[depth]}"/>
                                </span>
                            </div>
                        </g:each>
        ...
        <div class="buttons">
            <ul>
                <li><a class="submit save" onclick="nestedSearch();">
                    <span><g:message code="button.search"/></span>
                </a>
                </li>
                <g:if test="${nestedTables}">
                    <li><a class="submit save" onclick="$('#save-query-dialog').dialog('open');">
    ...
    %{-- Save query dialog --}%
    <div id="save-query-dialog" class="bg-lightbox" title="<g:message code="dataTables.query.save.title"/>"
         style="display:none;">
        <div id="save-query-dialog-errors" style="display: none; font-size: 12px;"></div>
        <g:formRemote name="save-query-confirm" url="[controller: 'route', action: 'saveQuery']" update="save-query-dialog-response" before="\$('#save-query-dialog-errors').empty();" after="checkSaveResponse();" asynchronous="false">
            <g:hiddenField name="nestedDepth" value="${nestedTables ? nestedTables.size() : 0}"/>
            <g:hiddenField name="rootTableId" value="${rootTableId}"/>
