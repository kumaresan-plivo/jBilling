...
class CompanyService implements Serializable {
    ...
    /**
     * Path on the disk where the entity logos are.
     * @return
     */
    def String generateEntityLogoPath() {
        return com.sapienter.jbilling.common.Util
                .getSysProp("base_dir") + "logos" + File.separator + "entity-${session['company_id']}.jpg";
    }

    /**
     * Save the entity logo to disk. Increase the version in memory
     *
     * @param f
     * @return
     */
    def saveEntityLogo(MultipartFile f) {
        f.transferTo(new File(generateEntityLogoPath()));
        entityLogoVersion[session['company_id']] = System.currentTimeMillis()
    }
