...
class RouteService implements Serializable {
        ...
        filters.each {
            if(it.value) {
                ...
                if(it.value instanceof List) {
                    ...
                    it.value = it.value as Integer
                } else {
                    constraint = FilterConstraint.ILIKE
                }
                if(it.value instanceof List) {
                    if(it.key == 'in.id') {
                        it.value = it.value.collect {e-> e as Integer}
                    }
                }
    ...
    static <T> T fetchData(SearchResult<String> result, RouteDTO routeDTO, String columnName, Class<T> type) {
        ...
        try{
            ...
            throw SessionInternalError("Data is not valid in data table : \""+routeDTO.getName()+"\". \n Not able to parse "+routeDTO.getName()+"."+columnName+ " value into type \""+type.toString()+"\"")
        }
